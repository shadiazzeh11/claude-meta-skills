...
| Live dogfood coverage | `./testing/analyze-log.py --real-only` shows controlled live-session evidence for all five hooks. The active post-`v0.1.2` window shows 29 real fires across 5 hooks, 10 sessions, and 8 projects, including the 6-fire clean disposable pass, four stale-installed-hook `completion-verifier` false positives before doctor/reinstall, post-reinstall LOGOS broken-state and `context-recovery` proofs, one live transcript-shape `completion-verifier` proof after PR #47, `v0.1.5` self and marketplace smoke, a long-session ledger stress run with 5 fires across 4 hooks, the LOGOS Phase C/Gamma-audit stress pass that exposed the untracked-file recovery gap, and a disposable untracked-file smoke that exposed copied-hook and settings-backup install noise. |
