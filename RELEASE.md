...
Current published release: **`v0.1.6` technical preview**.
...
VERSION=v0.1.7  # example next target; replace with the intended release version
...
- `make test-stop-env` passes `94/94`
...
- 94/94 synthetic harness validation tests.
- Controlled live Claude Code session evidence for all five hooks.
- Local installer/uninstaller with lifecycle tests.
- GitHub Actions validation on PRs and main across Ubuntu and macOS.
- Plugin scaffold and local marketplace catalog validation.
- New-user smoke protocol for external testers.
- Stop-hook inconclusive warnings aligned with Claude Code's current `systemMessage` output support.
