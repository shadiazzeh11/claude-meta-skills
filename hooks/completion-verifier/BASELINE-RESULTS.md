...
Initial validation from Phase 2 build, expanded in Phase 2.5 (transcript parsing + cargo-missing tests), Phase 2B dogfood follow-up (pytest-configured local-venv projects), the missing-pytest warning regression, timeout-env hardening, subdirectory project-root discovery, timeout partial-output warnings, and transcript schema tolerance for `tool_call` wrapper shapes. Re-run via `cd validation && ./harness.sh completion-verifier`.

| Metric | Value |
|---|---|
| Test date | 2026-05-11 (transcript schema tolerance update) |
...
| False positives | 0 |
| False negatives | 0 |
| Avg duration / case | 240 ms |
| Min duration / case | 71 ms |
| Max duration / case | 2089 ms (test 08 — timeout, waits 2s) |
| Total duration | 5771 ms |
...
| 08 | timeout | should-pass (warn) | 0 | 2089 | sleep 5 + timeout=2; emits warning with partial pre-timeout output |
...
| 12 | cargo-not-installed | should-pass (warn) | 0 | 221 | FileNotFoundError handler emits command-not-found warning |
...
| 15 | pytest-declared-missing | should-pass (warn) | 0 | 122 | pytest declared but unavailable; emits command-not-found warning without crashing |
...
- Anti-loop check (test 07) returns in 72ms in this run — under load this matters because the hook fires on every Stop attempt during forced-continuation.
