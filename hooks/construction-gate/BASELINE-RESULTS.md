...
| Metric | Value |
|---|---|
| Test date | 2026-05-09 (Phase 2B.3 update) |
| Claude Code version | 2.1.128 |
| Python | 3.14.2 |
| OS | Darwin 25.3.0 |
| Test cases | 19 (16 should-block, 2 should-pass, 1 edge case) |
| Pass rate | 19 / 19 |
| False positives | 0 |
| False negatives | 0 |
| Avg duration / case | 72 ms |
| Min duration / case | 69 ms |
| Max duration / case | 86 ms |
| Total duration | 1371 ms |

## Per-case results

| # | Case | Category | Tool | Exit | ms | Notes |
|---|---|---|---|---|---|---|
| 01 | node-modules | should-block | Write | 2 | 86 | Write deep into node_modules path; matches `node_modules/` pattern |
| 02 | env-file | should-block | Write | 2 | 70 | Write to .env.production; matches `\.env(?:\.\|$)` pattern |
| 03 | git-internals | should-block | Write | 2 | 73 | Write deep into .git/objects/pack/; matches `\.git/` pattern; confirms regex matches anywhere in path |
| 04 | normal-path | should-pass | Write | 0 | 69 | Write to src/app.py; no protected pattern matches |
| 05 | tmp-scratch | should-pass | Write | 0 | 70 | Write to /tmp/scratch.txt; no protected pattern matches |
| 06 | filename-contains-protected-string | should-pass | Write | 0 | 70 | Filename "my-node_modules-notes.md" contains "node_modules" substring but no `/` boundary; pattern correctly does NOT match (boundary test) |
| 07 | invalid-regex | should-block | Write | 2 | 71 | Hook skips invalid regexes gracefully and still enforces the valid `node_modules/` pattern; doesn't crash |
| 08 | package-lock | should-block | Write | 2 | 72 | Write to package-lock.json; matches `package-lock\.json$` pattern |
| 09 | yarn-lock | should-block | Write | 2 | 76 | Write to yarn.lock; matches `yarn\.lock$` pattern |
| 10 | bun-lockb | should-block | Write | 2 | 70 | Write to bun.lockb; matches `bun\.lockb$` pattern |
| 11 | claude-settings | should-block | Edit | 2 | 72 | Edit on .claude/settings.json — exercises Edit tool with file_path; matches `\.claude/settings\.json$` pattern |
| 12 | cargo-lock | should-block | Write | 2 | 71 | Write to Cargo.lock; matches `Cargo\.lock$` pattern |
| 13 | gemfile-lock | should-block | Write | 2 | 71 | Write to Gemfile.lock; matches `Gemfile\.lock$` pattern |
| 14 | poetry-lock | should-block | Write | 2 | 70 | Write to poetry.lock; matches `poetry\.lock$` pattern |
| 15 | uv-lock | should-block | Write | 2 | 70 | Write to uv.lock; matches `uv\.lock$` pattern |
| 16 | pnpm-lock | should-block | Write | 2 | 70 | Write to pnpm-lock.yaml; matches `pnpm-lock\.yaml$` pattern (Phase 2B.3) |
| 17 | pipfile-lock | should-block | Write | 2 | 80 | Write to Pipfile.lock; matches `Pipfile\.lock$` pattern (Phase 2B.3) |
| 18 | claude-settings-local | should-block | MultiEdit | 2 | 70 | MultiEdit on .claude/settings.local.json — exercises MultiEdit tool with file_path; matches `\.claude/settings\.local\.json$` pattern (Phase 2B.3) |
| 19 | claude-hooks-dir | should-block | NotebookEdit | 2 | 70 | NotebookEdit payload uses `notebook_path` fallback (no `file_path` provided); matches `\.claude/hooks/` pattern (Phase 2B.3) |

## Notes on performance

- All cases consistently 69-86ms. Hook is essentially Python startup + JSON parsing + regex compilation + path matching.
...
- Constructive stderr message verified on all 16 should-block cases: contains the matched pattern and explanation.
...
- Tool coverage: cases 01–10/12–17 use `Write`, case 11 uses `Edit`, case 18 uses `MultiEdit`, case 19 uses `NotebookEdit` (exercising the `tool_input.notebook_path` fallback when `file_path` is absent).
