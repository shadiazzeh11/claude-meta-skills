...
| Max duration / case | 168 ms (test 07 — token-budget) |
| Total duration | 1563 ms |

## Per-case results

| # | Case | Category | Exit | ms | Notes |
|---|---|---|---|---|---|
| 01 | git-repo-with-claude-md | should-write | 0 | 152 | git context captured + appended to existing CLAUDE.md |
| 02 | non-git-directory | should-write | 0 | 71 | reminders + timestamp only; no git fields |
...
| 05 | content-updates | should-write | 0 | 133 | stale "Branch: stale-branch" replaced with current branch info |
| 06 | read-only-claude-md | should-pass-silently | 0 | 72 | read-only file + read-only parent dir; hook exits 0 without crashing; original unchanged |
| 07 | token-budget | should-write | 0 | 168 | 200 in-progress files; recovery section truncated to fit ~2000-char budget |
| 08 | claude-project-dir-env | should-write | 0 | 76 | CLAUDE_PROJECT_DIR env var override; hook writes to env path, not cwd |
| 09 | custom-instructions-redacted | should-write | 0 | 141 | manual compact instructions redact secret-like assignments while preserving safe sentinel text |
| 10 | custom-instructions-hard-cap | should-write | 0 | 76 | oversized custom instructions cannot push the recovery block beyond the configured hard cap |
...
| 12 | subdir-git-root-no-claude-md | should-write | 0 | 132 | no env var; nested cwd creates CLAUDE.md at nearest parent git root |
...
| 14 | tracked-and-untracked-files | should-write | 0 | 132 | tracked modification and untracked non-ignored file included; ignored file excluded |

## Notes on performance

- Median per-case ~132ms. Tests with no git work (02, 06, 08, 10, 13) complete fastest. Tests with git command suites and fixture setup (01, 03, 05, 07, 11, 12, 14) are slower.
- Test 07 (token-budget) is the max at 168ms in this run. All cases remain under the 500ms target.
...
- In-progress file recovery verified (test 14): the recovery block includes tracked modifications and untracked non-ignored files while excluding ignored local files.
