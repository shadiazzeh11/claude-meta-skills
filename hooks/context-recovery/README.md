...
- **Copied install artifacts excluded.** Local `install.sh` copies this suite into `.claude/hooks/meta-skills/` and may leave timestamped `.claude/settings.json.backup-*` files. Those are installation noise, not task context, so the recovery list excludes them to keep real in-progress work visible under the token budget. The active `.claude/settings.json` file remains visible.
